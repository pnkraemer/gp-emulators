...
cov_fct = MaternCov.fast_mtrx
polblocksize = 0

precon_vals, precon_rowidx, precon_colidx, num_neighb = LocalLagrange.precon(ptset, loc_radius, cov_fct, polblocksize)

precon = scipy.sparse.coo_matrix((precon_vals, (precon_rowidx, precon_colidx)), shape=(num_pts, num_pts))
...
if save_txt == True:
	np.savetxt("precon_txt_matern_square/ptSet_Halton_N%d_n%d.txt"%(num_pts, num_neighb), ptset, fmt='%.17e')
	np.savetxt("precon_txt_matern_square/preconVals_N%d_n%d.txt"%(num_pts, num_neighb), precon_vals, fmt='%.17e')
	np.savetxt("precon_txt_matern_square/preconRowIdx_N%d_n%d.txt"%(num_pts, num_neighb), precon_rowidx, fmt='%d')
	np.savetxt("precon_txt_matern_square/preconColIdx_N%d_n%d.txt"%(num_pts, num_neighb), precon_colidx, fmt='%d')
	print('\n\nSaving complete\n')
print()
