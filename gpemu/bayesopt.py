...
class BayesOpt:
	...
	def __init__(self, cgp, acq = 'PI', thresh = 0.0, minimiser = "LBFGS", num_restarts = 10):
		...
		self.last_argmax = self.mesh[np.argmax(self.cond_gp.mean_fct.evaluate(self.mesh))].reshape((1,len(self.mesh.T)))
	...
	def maximize(self, tolerance):
		...
		spatial_dim = len(self.mesh.T)
		assert(spatial_dim == 2), "dim > 2 not initialized yet"
		...
		if self.minimiser == "DIRECT":
			...
			resx = res.x.reshape((1,2))
		elif self.minimiser == "LBFGS":
			minval = 1
			for i in range(self.num_restarts):
				x0 = np.random.rand(1,2)
				bds = ((0, 1), (0, 1),)
				res = minimize(objective, x0, bounds = bds, tol = tolerance, method='L-BFGS-B')
				if res.fun < minval:
					minval = res.fun[0]
...
class BayesOpt2d:
	...
	def maximize(self, tolerance):
		...
		if self.minimiser == "DIRECT":
			res = scdrmin(objective, bounds = [(0, 1), (0,1)])#, tol = tolerance)
			resx = res.x.reshape((1,2))
		elif self.minimiser == "LBFGS":
			minval = 1
			for i in range(self.num_restarts):
